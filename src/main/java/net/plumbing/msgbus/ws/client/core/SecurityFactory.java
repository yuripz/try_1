...
public class SecurityFactory {
    ...
    private void configureAuthentication(Security.Builder builder) {
        if(authMethod != null) {
            SoapConstants.AuthMethod method = SoapConstants.AuthMethod.valueOf(authMethod);
            if(method.equals(SoapConstants.AuthMethod.BASIC)) {
                builder.authBasic(authUsername, authPassword);
            } else if(method.equals(SoapConstants.AuthMethod.DIGEST)) {
                builder.authDigest(authUsername, authPassword);
            } else if(method.equals(SoapConstants.AuthMethod.NTLM)) {
                builder.authNtlm(authUsername, authPassword, authWorkstation, authDomain);
            } else if(method.equals(SoapConstants.AuthMethod.SPNEGO)) {
                builder.authSpnego();
            }
        }
    }
