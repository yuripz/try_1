...
public class InitMessageRepository {
    ...
    public static  int ReReadMsgTemplates(Logger AppThead_log )  {
        ...
        try {
            AppThead_log.info("SELECT * FROM ARTX_PROJ.MESSAGE_TemplateS T where T.LastDate > (to_date('" + DataAccess.dateFormat.format( DataAccess.InitDate )  +"', 'YYYY-MM-DD HH24:MI:SS' ) - 1/(60*24) ) ;" );
            ...
            try {
                stmtMsgTemplateReRead = DataAccess.Hermes_Connection.prepareStatement(
                        "select t.template_id, " +
                                ...
                                "where (1=1) and t.template_dir like '%IN%' " + // "and t.operation_id =0 " +
                                "and t.LastDate > ( ? - 2/(60*24)) " + //"and t.LastDate > to_date( ?, 'YYYY-MM-DD HH24:MI:SS) " +
